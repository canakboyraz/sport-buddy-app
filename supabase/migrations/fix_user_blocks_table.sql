...
DROP FUNCTION IF EXISTS are_users_blocking(uuid, uuid);
DROP FUNCTION IF EXISTS block_user(uuid, uuid, text);
DROP FUNCTION IF EXISTS unblock_user(uuid, uuid);
DROP FUNCTION IF EXISTS get_blocked_users(uuid);
...
BEGIN
  ...
  -- Remove friendship if exists (table may not exist, so we use IF EXISTS)
